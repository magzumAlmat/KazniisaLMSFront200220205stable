...
export default function MaterialsPage() {
  ...
  const [lesson_id, setLessonId] = useState("");
  const [editingMaterial, setEditingMaterial] = useState(null);
  const [files, setFiles] = useState([]); // For video files
  const [documentFiles, setDocumentFiles] = useState([]);
  const [imageFiles, setImageFiles] = useState([]);
  const [audioFiles, setAudioFiles] = useState([]); // For WAV files
  const [presentationFiles, setPresentationFiles] = useState([]);
  const [courses, setCourses] = useState([]);
  const [testFilePath, setTestFilePath] = useState("");
  const [userInfo, setUserInfo] = useState(null);
  ...
  // Dropzone setup for presentations
  const { getRootProps: getPresentationRootProps, getInputProps: getPresentationInputProps } = useDropzone({
  ...
  const createMaterial = async () => {
    ...
    if (
      ...
      (type === "document" && documentFiles.length === 0) ||
      (type === "presentation" && presentationFiles.length === 0) ||
      (type === "audio" && audioFiles.length === 0) || // Validate audio files
      ((type === "test" || type === "opros") && testFilePath.trim() === "")
    ) {
      alert("Выберите файл или укажите ссылку на тест/опрос!");
      return;
    }
    ...
    try {
      ...
      if (type === "video") {
        ...
        const formData = new FormData();
        formData.append("file", audioFiles[0]);
        formData.append("name", title);
        uploadedFileResponse = await axios.post(`${host}/api/upload`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      } else if ((type === "test" || type === "opros") && testFilePath.trim() === "") {
        alert("Введите ссылку на тест или опрос!");
        return;
      }
      ...
      setDocumentFiles([]);
      setPresentationFiles([]);
      setAudioFiles([]); // Reset audio files
      setTestFilePath("");
      window.location.reload();
    } catch (error) {
      console.error("Ошибка при создании материала:", error);
          ...
          <Box mt={4}>
            ...
            <FormControl fullWidth sx={{ mt: 2 }}>
              ...
              <Select value={type} onChange={(e) => setType(e.target.value)} fullWidth required>
                ...
                <MenuItem value="document">Документ</MenuItem>
                <MenuItem value="presentation">Презентация</MenuItem>
                <MenuItem value="audio">Аудио (WAV)</MenuItem> {/* Added audio option */}
                <MenuItem value="test">Ссылка на тест</MenuItem>
                <MenuItem value="opros">Ссылка на опрос</MenuItem>
              </Select>
            </FormControl>
            ...
            {type === "video" && (
              <Box mt={2}>
                <div {...getVideoRootProps()}>
                  <input {...getVideoInputProps()} />
                  <Typography>Перетащите видео сюда или нажмите для выбора</Typography>
                </div>
                {files.length > 0 && <Typography>Выбранный файл: {files[0].name}</Typography>}
              </Box>
            )}
            ...
            {type === "audio" && (
              <Box mt={2}>
                <div {...getAudioRootProps()}>
                  ...
                  <Typography>Перетащите WAV аудиофайл сюда или нажмите для выбора</Typography>
                </div>
                {audioFiles.length > 0 && (
                  <Typography>Выбранный файл: {audioFiles[0].name}</Typography>
                )}
                {audioFiles.length > 0 && (
                  <Box mt={2}>
                    <audio controls src={audioFiles[0].preview}>
