...
//                   width: "100%",
//                   maxWidth: "100%",
//                   height: "54px", // Standard height for audio controls
//                 }}
//               >
//                 <source src={updatedFilePath} type="audio/wav" />
    ...
    MuiTab: {
      styleOverrides: {
        root: {
          ...
          "&.Mui-selected": {
            color: "#009eb0",
            backgroundColor: "rgba(0, 158, 176翻译成中文是：0.15)",
          },
        },
      },
    },
...
//     return <Typography sx={{ color: theme.palette.text.secondary, fontSize: { xs: "0.75rem", sm: "0.875rem" } }}>Видео недоступно.</Typography>;
//   }
//   const updatedFilePath = getUpdatedFilePath(material.file_path);
//   return (
...
// const getUpdatedFilePath = (filePath) => {
//   console.log("getUpdatedFilePath started", filePath);
//   // Return filePath as-is, assuming backend provides correct URL
//   return filePath;
// };
...
export default function CourseDetail() {
  ...
  // Формирование массива блоков с приоритетами
  const contentBlocks = useMemo(() => {
    console.log("Filtered materials:", filteredMaterials); // Debug materials
    const blocks = [];
          ...
          <Box key="materials" sx={{ width: "100%", maxWidth: "100%", overflow: "hidden" }}>
            ...
            {filteredMaterials.length > 0 ? (
              <MuiList sx={{ p: 0 }}>
                {filteredMaterials.map((material) => {
                  const updatedFilePath = material.file_path ? getUpdatedFilePath(material.file_path) : null;
                  console.log("Material:", material.material_id, "Type:", material.type, "FilePath:", updatedFilePath);
                  ...
                  return (
                    <Box key={material.material_id} sx={{ mb: 2 }}>
                      ...
                      >
                        ...
                        {material.type === "audio" && updatedFilePath ? (
                          <Box sx={{ mt: { xs: 1, sm: 0 }, width: "100%", maxWidth: { xs: "100%", sm: "400px" } }}>
                            <audio
                              controls
                              src={updatedFilePath}
                              style={{
                        ...
                        ) : material.type === "image" && updatedFilePath ? (
                          <Box sx={{ mt: { xs: 1, sm: 0 }, width: "100%", maxWidth: { xs: "100%", sm: "400px" } }}>
                            <img
                              src={updatedFilePath}
                              alt={material.title}
                          ...
                          // Videos are handled in videoMaterials block
                          null
                          ...
                          updatedFilePath ? (
                            ...
                            >
                              <Button
                                ...
                                sx={{ ml: { xs: 0, sm: 1.5 }, fontSize: { xs: "0.625rem", sm: "0.75rem" }, width: { xs: "100%", sm: "auto" } }}
                              >
                                Перейти к {material.type === "test" ? "тесту" : "опросу"}
                              </Button>
                            </a>
                          ) : (
                            <Typography
        ...
        <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, minHeight: "calc(100vh - 64px)", p: { xs: 1.5, sm: 2 } }}>
          ...
          <Box sx={{ flexGrow: 5, width: "100%", maxWidth: "100%", boxSizing: "border-box", mr: { xs: 0, sm: 3 } }}>
            <Paper elevation={3} sx={{ p: { xs: 2, sm: 5.5 }, bgcolor: theme.palette.background.paper, width: "90%", maxWidth: "100%", overflow: "hidden" }}>
              ...
              <Typography variant="h6" sx={{ color: theme.palette.text.primary, fontSize: { xs: "1rem", sm: "1.25rem" }, mb: 1.5 }}>
                {filteredLessons[activeTab]?.title || "Урок"}
              </Typography>
